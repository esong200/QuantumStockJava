...
public class DoubleLayer extends functions {
	...
	public static void main(String[] args) {
		...
		for(int m = 0; m<100000; m++) {
			...
			for (int i = 0; i<3; i++) {
				for(int j = 0; j<4; j++) {
					synapticWeights0[i][j] += dotMultiplyFastest(inputs, delta0 )[i][j];
				}
			}
			System.out.println();
		}
		...
		double[] answers = sigmoid1d(dotMultiply(synapticWeights1, sigmoid(dotMultiplyFastest(testArr, synapticWeights0), false)), false);

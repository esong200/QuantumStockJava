...
public class TripleLayerSequential {
	public static void main(String[] args) {
		String comp = "AMZN";
		String runName = "ShortTimeSequential";
		...
		for(int m = 0; m<1000000; m++) {
			...
			if(dataIndex >= dataTaylored.size()-2) {
				dataIndex = 0;
			}
			if(ansIndex >= dataTaylored.size()-2) {
				ansIndex = 0;
			}
			
			for(int i=0; i<dataTaylored.get(0).length; i++) {
				inputs[i] = dataTaylored.get(dataIndex)[i];
			}
			for(int i=0; i<ans.get(0).length; i++) {
				desiredOutcome[i] = ans.get(ansIndex)[i];
			}

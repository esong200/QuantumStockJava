...
public class AlphaVantageCollectorWeeklyChunk extends CSVReadWrite{
	...
	public static ArrayList<double[][]> weeklyData (String symbol){
		...
		try {
			...
			double[][] inputData = new double[4][5];
				stockData.forEach(stock -> {
					for(int i=0; i<4; i++) {
						inputData[i][0] = stock.getOpen();
						inputData[i][1] = stock.getHigh();
						inputData[i][2] = stock.getLow();
						inputData[i][3] = stock.getClose();
						inputData[i][4] = stock.getVolume();
						data.add(inputData);
					}
				});
	...
	public static ArrayList<double[][]> answerCompile (String symbol){
		...
		for(int i = 0; i < weekly.get(i).length -1; i++){
